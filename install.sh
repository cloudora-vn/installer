...
AVAILABLE_LANGS=("en" "zh" "vi" "fa" "pt-BR" "ru")

declare -A LANG_NAMES
LANG_NAMES=( ["en"]="English" ["zh"]="Chinese  中文(简体)" ["vi"]="Tiếng Việt" ["fa"]="Persian" ["pt-BR"]="Português (Brasil)" ["ru"]="Русский" )
...
function Prepare_System(){
    if which corapanel-core >/dev/null 2>&1; then
...
function Set_Dir(){
    ...
    if [[ -f "$PANEL_BASE_DIR/corapanel/db/core.db" ]]; then
        USE_EXISTING=true
    fi
}
...
BACKUP_FILE="/etc/docker/daemon.json.corapanel_bak"
...
function Init_Panel(){
    ...
    RUN_BASE_DIR=$PANEL_BASE_DIR/corapanel
    mkdir -p "$RUN_BASE_DIR"
    rm -rf "$RUN_BASE_DIR:?/*"

    cd "${CURRENT_DIR}" || exit

    cp ./corapanel-core /usr/local/bin && chmod +x /usr/local/bin/corapanel-core
    if [[ -e /usr/bin/corapanel ]]; then
        rm -f /usr/bin/corapanel
    fi
    ln -s /usr/local/bin/corapanel-core /usr/bin/corapanel >/dev/null 2>&1
    if [[ ! -f /usr/bin/corapanel-core ]]; then
        ln -s /usr/local/bin/corapanel-core /usr/bin/corapanel-core >/dev/null 2>&1
    fi

    cp ./corapanel-agent /usr/local/bin && chmod +x /usr/local/bin/corapanel-agent
    if [[ ! -f /usr/bin/corapanel-agent ]]; then
        ln -s /usr/local/bin/corapanel-agent /usr/bin/corapanel-agent >/dev/null 2>&1
    fi

    cp ./corapctl /usr/local/bin && chmod +x /usr/local/bin/corapctl
    sed -i -e "s#BASE_DIR=.*#BASE_DIR=${PANEL_BASE_DIR}#g" /usr/local/bin/corapctl
    sed -i -e "s#ORIGINAL_PORT=.*#ORIGINAL_PORT=${PANEL_PORT}#g" /usr/local/bin/corapctl
    sed -i -e "s#ORIGINAL_USERNAME=.*#ORIGINAL_USERNAME=${PANEL_USERNAME}#g" /usr/local/bin/corapctl
    ESCAPED_PANEL_PASSWORD=$(echo "$PANEL_PASSWORD" | sed 's/[!@#$%*_,.?]/\\&/g')
    sed -i -e "s#ORIGINAL_PASSWORD=.*#ORIGINAL_PASSWORD=${ESCAPED_PANEL_PASSWORD}#g" /usr/local/bin/corapctl
    sed -i -e "s#ORIGINAL_ENTRANCE=.*#ORIGINAL_ENTRANCE=${PANEL_ENTRANCE}#g" /usr/local/bin/corapctl
    sed -i -e "s#LANGUAGE=.*#LANGUAGE=${selected_lang}#g" /usr/local/bin/corapctl
    if [[ "$USE_EXISTING" == true ]]; then
        if grep -q "^CHANGE_USER_INFO=" "/usr/local/bin/corapctl"; then
            sed -i 's/^CHANGE_USER_INFO=.*/CHANGE_USER_INFO=use_existing/' "/usr/local/bin/corapctl"
        else
            sed -i '/^LANGUAGE=.*/a CHANGE_USER_INFO=use_existing' "/usr/local/bin/corapctl"
        fi
    fi
    if [[ ! -f /usr/bin/corapctl ]]; then
        ln -s /usr/local/bin/corapctl /usr/bin/corapctl >/dev/null 2>&1
    fi
    ...
    cp ./corapanel-core.service /etc/systemd/system
    cp ./corapanel-agent.service /etc/systemd/system

    systemctl enable corapanel-agent.service; systemctl enable corapanel-core.service; systemctl daemon-reload 2>&1 | tee -a "${CURRENT_DIR}"/install.log
    log "$TXT_START_PANEL_SERVICE"
    systemctl start corapanel-core | tee -a "${CURRENT_DIR}"/install.log
    systemctl start corapanel-agent | tee -a "${CURRENT_DIR}"/install.log

    for i in {1..30}; do
        sleep 3

        core_status=$(systemctl status corapanel-core 2>&1 | grep Active)
        agent_status=$(systemctl status corapanel-agent 2>&1 | grep Active)
...
function Check_Ready() {
    ...
    if [[ "$USE_EXISTING" == false ]]; then
        sed -i -e "s#ORIGINAL_PASSWORD=.*#ORIGINAL_PASSWORD=\*\*\*\*\*\*\*\*\*\*#g" /usr/local/bin/corapctl
    fi
}
